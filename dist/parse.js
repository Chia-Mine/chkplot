...
exports.parsePlotterLog = void 0;
function parsePlotterLog(log) {
    var _a;
    ...
    for (let l = 0; l < maxLine; l++) {
        ...
        if (currentPhase === "starting") {
            ...
            if (match) {
                ...
                phase1[subState] = phase1[subState] || {};
                continue;
            }
            const regexForBuckets = /^\tBucket (\d+) (.+)\. Ram: ([\d.]+)(.+), u_sort min: ([\d.]+)(.+), qs min: ([\d.]+)(.+)\.( force_qs: (.*))?$/;
            match = regexForBuckets.exec(line);
            if (match) {
                phase1[subState].bucketInfo = ((_a = (phase1[subState])) === null || _a === void 0 ? void 0 : _a.bucketInfo) || [];
                const bucketInfo = {
                    bucket: +match[1],
                    sort: match[2],
                    ram: [+match[3], match[4]],
            ...
            if (match) {
                ...
                phase3[subState] = phase3[subState] || {};
                continue;
            }
            const regexForBuckets = /^\tBucket (\d+) (.+)\. Ram: ([\d.]+)(.+), u_sort min: ([\d.]+)(.+), qs min: ([\d.]+)(.+)\.( force_qs: (.*))?$/;
            match = regexForBuckets.exec(line);
            if (match) {
                phase3[subState].bucketInfo = phase3[subState].bucketInfo || [];
                const bucketInfo = {
                    bucket: +match[1],
                    sort: match[2],
                    ram: [+match[3], match[4]],
                ...
                (phase3[subState].bucketInfo).push(bucketInfo);
                continue;
            }
            ...
            if (match) {
                phase3[subState].firstComputationSummary = {
                    time: +match[1],
                    cpu: +match[2],
                    date: new Date(match[3]),
                };
            ...
            if (match) {
                phase3[subState].secondComputationSummary = {
                    time: +match[1],
                    cpu: +match[2],
                    date: new Date(match[3]),
                };
...
exports.parsePlotterLog = parsePlotterLog;
